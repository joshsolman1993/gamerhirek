...
import { sendGuildMessage } from "@/actions/social";
import { SendIcon, Loader2 } from "lucide-react";

export function GuildChatInput({ guildId }: { guildId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const result = await sendGuildMessage(guildId, content);
    setLoading(false);
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
      <input
        ...
        placeholder="Írj a klántagoknak..."
        style={{
          flex: 1,
